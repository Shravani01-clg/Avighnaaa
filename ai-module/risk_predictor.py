...
class RiskPredictor:
    ...
    def predict(self, sensor_reading: dict) -> dict:
        ...
        # Get encoded prediction from the ML model
        prediction_encoded = self.model.predict(X_scaled)[0]

        # Convert encoded NumPy value into actual risk label
        # and explicitly convert to Python string
        prediction = str(
        ...
        for i, class_name in enumerate(
            ...
            prob_dict[str(class_name)] = round(
        ...
        # Risk score based on predicted level
        risk_score_map = {
            "LOW": 15,
            "MEDIUM": 45,
            "HIGH": 70,
        ...
        # Explicit Python int for JSON compatibility
        risk_score = int(
            risk_score_map.get(prediction, 15)
        )

        # Adjust score within level based on probability distribution
        if prediction != "LOW":

            # If there is significant probability
            # of a higher level, boost the score
            for level in [
                "MEDIUM",
                "HIGH",
                "CRITICAL"
                ...
                if level in self.label_encoder.classes_:
    ...
    def load(self, path=None):
        ...
        print(
            f"Risk predictor loaded from {path}"
        )
