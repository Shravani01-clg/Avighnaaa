        ...
        for (const worker of workers) {
            ...
            if (latestSensor.sos === true) {
                ...
                appState.alerts.unshift({
                    ...
                    aiAction:
                        "Contact the worker immediately and dispatch emergency assistance to the reported location.",
        ...
        body.innerHTML = `
            ...
            <div class="detail-grid">
                ...
                >
                    ...
                    <div class="detail-value">
                        ${
                            d.acceleration_x_ms2.toFixed(
                                3
                            )
                        },
                        ${
                            d.acceleration_y_ms2.toFixed(
                                3
                            )
                        },
                        ${
                            d.acceleration_z_ms2.toFixed(
                                3
                            )
                        }
                        m/s&sup2;
...
function logStatusChange(
    ...
    if (
        ...
        addLogEntry(
            'STATUS',
            'tag-status',
            `${workerId} status changed: ${fromStatus} → ${toStatus}${
                detail
                    ? ' (' +
                      detail +
                      ')'
...
window.pingBelt =
    function () {
        ...
        btn.innerHTML =
            '<i data-lucide="loader-2" class="spin"></i> Pinging...';
        ...
        if (!worker) {
            ...
            btn.innerHTML =
                '<i data-lucide="send"></i> Ping Belt';
        ...
        addLogEntry(
            'PING',
            'tag-ping',
            `Belt ${d.device_id} pinged – RSSI OK, last data ${d.temperature_c.toFixed(
                1
            )}°C, ${Math.round(
                d.gas_raw
            )} gas, ${d.water_level_cm.toFixed(
                1
            )}cm water`
        );
