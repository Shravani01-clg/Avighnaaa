...
function generateNormal() {
  ...
  state.batteryLevel = clamp(
    state.batteryLevel - 0.02,
    0,
    100
  );
  ...
  state.waterLevelCm = clamp(
    state.waterLevelCm,
    normal.waterLevelCm.min,
    normal.waterLevelCm.max
  );
  ...
  return {
    ...
    temperature_c:
      Math.round(state.temperature * 10) / 10,

    gas_raw:
      Math.round(state.gasRaw),

    water_level_cm:
      Math.round(state.waterLevelCm * 10) / 10,

    battery_level_percent:
      Math.round(state.batteryLevel * 100) / 100,

    acceleration_x_ms2:
      Math.round(state.accelX * 100) / 100,

    acceleration_y_ms2:
      Math.round(state.accelY * 100) / 100,

    acceleration_z_ms2:
      Math.round(state.accelZ * 100) / 100,
 ...
 *
 * Simulates:
 * normal → free-fall → impact → lying down
 *
...
function generateFall(phase) {
  ...
  switch (phase) {
    ...
    case 1:
      ...
      reading.acceleration_z_ms2 = jitter(0.5, 0.5);
      break;

    case 2:
      // Impact
      reading.acceleration_x_ms2 = jitter(25, 5);
      reading.acceleration_y_ms2 = jitter(20, 5);
      reading.acceleration_z_ms2 = jitter(35, 8);
      break;

    case 3:
      // Post-fall
      reading.acceleration_x_ms2 = jitter(6.0, 1.0);
      reading.acceleration_y_ms2 = jitter(1.5, 0.5);
      reading.acceleration_z_ms2 = jitter(4.0, 1.0);
      break;
  }
...
/**
 * Generate a random danger event
 * (2% chance per normal reading)
 */
function maybeRandomEvent() {
  if (Math.random() < config.randomEventChance) {
    ...
    return {
      type: "danger",
      dangerType: event,
      reading: generateDanger(event, 0.8),
    };
  }
